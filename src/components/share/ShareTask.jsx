...
const ShareTask = () => {
  ...
  const countCheckedItems = (checklist) => {
    return checklist.reduce((count, item) => count + (item.isChecked ? 1 : 0), 0);
  };
  ...
  return (
    <>
      ...
      <div className={styles.container}>
        <span>
          <img src={getPriorityColor(task.priority)} alt={`${task.priority} priority`} />
        </span>
        <span className={styles.prior}>{task.priority?.toUpperCase()} PRIORITY</span>
        <h2>{task.title}</h2>
        <div className={styles.checklistContainer}>
          <h4>
            Checklist ({countCheckedItems(task.checklist)}/{task.checklist.length})
          </h4>
          <ul>
